...
export default function AttendancePage({ user }: { user: any }) {
    ...
    const handleClockIn = async () => {
        ...
        if (!location) {
            setStatus('error');
            setMessage('Gagal mendapatkan lokasi GPS.');
            return;
        }

        setLoading(true);
        const imageToSubmit = capturedImage || (videoRef.current ? capture() : null);
                ...
                <div className="space-y-4 md:space-y-6">
                    <div className="rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 bg-white p-5 md:p-8 text-center shadow-lg md:shadow-2xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
                        ...
                        <div className="space-y-0.5 md:space-y-1">
                            <span className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Waktu Sekarang</span>
                        ...
                        <div className={cn(
                            ...
                            {location && (
                                <p className="text-[8px] md:text-[9px] font-black opacity-60 tracking-widest uppercase">
                            ...
                            {status === 'success' ? (
                                <div className="animate-in zoom-in duration-500 text-emerald-600 flex flex-col items-center space-y-3 py-4">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Terima kasih atas dedikasi Anda hari ini.</p>
                                    </div>
                                </div>
                            ) : (
                                <>
